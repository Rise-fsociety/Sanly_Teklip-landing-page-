...
import styles from './appearTitle.module.css';
...
const AppearTitle = forwardRef<any, AppearTitleProps>(({ 
  children, 
  index = -1, 
  optionIndex = -1, 
  isFooter = false 
    ...
    if (containerRef.current) {
      ...
      childArray.forEach((child, i) => {
        if (child instanceof HTMLElement) {
          ...
          if (!isFooter && containerRef.current) {
    ...
    <div
      ref={(node) => {
        ...
        if (ref && typeof ref !== 'function' && ref.current && index !== -1 && optionIndex !== -1) {
          // Handles the dynamic grid matrix array allocation safely
