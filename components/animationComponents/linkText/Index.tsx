...
interface LinkTextProps {
  children: ReactNode;
  href: string;
  svgX?: number | string; // Made optional with '?'
  spanX?: number | string; // Made optional with '?'
  title: string;
  className?: string;
  hovered?: boolean;
  target?: boolean;
...
function LinkText({
  children,
  href,
  svgX = 0, // Fallback default value
  spanX = 0, // Fallback default value
  title,
  className = undefined,
  hovered = undefined,
  target = false,
  ...
  const animateHover = (isHovered: boolean | undefined) => {
    if (!svgRef.current || !spanRef.current) return;
